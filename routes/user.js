...
router.get('/login', async(req,res)=>{
  if(req.session.loggedIn){
    ...
    let homeCategory = await userHelpers.getHomeCategories()
    let cartCount=0;
    res.render('user/login',{"loginErr":req.session.loginErr,"blockErr": req.session.blockErr,user:true,homeCategory,cartCount})
...
router.get('/categoryProducts/:category',async(req,res)=>{
  ...
  let product = await userHelpers.getProductsByCateogry(category)
    ...
    res.render('user/category-pro',{product,user1,category,homeCategory,cartCount,ordersCount});
})
...
router.get('/loginOtp',async(req,res)=>{
  ...
  let cartCount = 0
    ...
    res.render('user/user-mobile',{user:true,homeCategory,"blockErr": req.session.blockErr,"noUser": req.session.noUserMobile,cartCount})
...
router.get('/login/otp',async(req,res)=>{
  if(req.session.loggedIn){
    ...
    res.render('user/user-otp',{otp: true,homeCategory,"invalidOtp": req.session.invalidOtp,cartCount})
...
router.get('/singleOrder/:id', verifyLogin,async(req, res) => {
  ...
  adminHelpers.getOrderProducts(Id).then((products) => {
    res.render('user/single-orders', { products,user:true,user1,homeCategory,cartCount,ordersCount})
  })
})
...
router.get('/user-change-password',async(req,res)=>{
  ...
  res.render('user/user-change-password',{"notSame": req.session.pswdNotSame, "invalid": req.session.invalidpswd,user1,homeCategory,cartCount,ordersCount})
...
router.get('/add-new-addressProfile',verifyLogin,async(req,res)=>{
  ...
  res.render('user/add-new-addressProfile',{user1,homeCategory,cartCount,ordersCount,user:true})
})
...
router.get('/edit-address/:id',verifyLogin,async(req,res)=>{
  ...
  userHelpers.getSingleAddress(addId,userId).then((address)=>{
    console.log(address);
    res.render('user/edit-address',{userId,user:true,address,user1,homeCategory,cartCount,ordersCount})
